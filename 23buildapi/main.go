...
func main() {
	fmt.Println("API - with golang")
	r := mux.NewRouter()

	// seeding of data
	courses = append(courses, Course{CourseId: "2", CourseName: "ReactJS", CoursePrice: 299, Author: &Author{Fullname: "harshit", Website: "go.dev"}})
	courses = append(courses, Course{CourseId: "4", CourseName: "MERN", CoursePrice: 199, Author: &Author{Fullname: "harshit", Website: "mern.dev"}})
...
func createOneCourse(w http.ResponseWriter, r *http.Request) {
	...
	for _, c := range courses {
		if c.CourseName == course.CourseName {
			json.NewEncoder(w).Encode("course with coursename already exists")
			return
		}
	}
